...
export default class Page extends Laya.Sprite{
    ...
    pageHandle(data){    
        ...
        if(data.Prompt)
        {
            ...
            RoomScene.getInstance().creatIcon(data.HasAutoPrompt,this.Prompt);
        }
    ...
    showSceneItem(data){    
        ...
        if(this.ElementIdAction.length>0){  
             for(let i = 0;i < this.ElementIdAction.length;i++){
                 //驱动方式为自动时  直接出现  
                 if(this.ElementIdAction[i].DriveType === 0){
    ...
    addElementMedia(item){
        ...
        if(GM.ImageLibrary[item.MediaId].type === "audio"){          
            itemMedieButton = new audioIcon(item.MediaMode,GM.ImageLibrary[item.MediaId].Url,item.MediaDelay);  
    ...
    // 答案区满足的Eventlist
    showClearEventList(EventList,from){
        ...
        if(EventList.ElementId != "" && EventList.ElementId != null){
            ...
            Laya.timer.once(delay,this,()=>{
                // 元素出现
                this.showClearElement(ElementId,0,from);  
            });        
        }
    ...
    showClearElement(name,type,from){
        // 0是出现 1 是消失
        let ElementId;
        let sta = false;
