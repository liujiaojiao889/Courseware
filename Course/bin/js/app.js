...
{   
    ...
    //设置版本控制类型为使用文件名映射的方式
    ResourceVersion.type = Laya.net.ResourceVersion.FILENAME_VERSION;
    //加载版本信息文件
 ...
 */ 
{ 
    Laya.loader.load(LIBRARY.LoadingLIBRARY, Laya.Handler.create(this, () => {  
     ...
     switch(cmd){
        ...
        case "optStart": //声音
            if(data.type === "Prompt" &&　data.senderId != GM.senderId){
                 observer.publish("playSound",data.url,data.type);
            }          
            break;
        case "getAnswerList":
